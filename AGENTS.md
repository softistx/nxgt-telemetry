...
`bun run test` is **337 pass, 0 fail**: `@nxgt/telemetry` 238,
`@nxgt/telemetry-otlp` 72, `@nxgt/telemetry-hono` 22, scripts 5. It runs one process per package, then the
...
the retry policy and the three failures. `@nxgt/telemetry-hono` is complete: the
server span, the `traceparent` continuation, the route rename and the context
variables. The three remaining integration packages are being built on the
`feat/telemetry` integration branch, and nothing has been published yet.
